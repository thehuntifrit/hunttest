...
function findConsecutiveWeather(mob, pointSec, minRepopSec, limitSec) {
  ...
  // 1. 巻き戻し探索: searchPoint から requiredSec 分遡る
  ...
  // checkStart から searchPoint 付近まで連続カウント
  while (cursor <= searchPoint) {
    ...
    if (checkWeatherInRange(mob, seed)) {
      ...
      // requiredCycles に達したかチェック
      if (consecutiveCount >= requiredCycles) {
        const windowEnd = cursor + WEATHER_CYCLE_SEC;

        // searchPoint が windowEnd 以前なら条件成立
        if (searchPoint <= windowEnd && searchPoint >= consecutiveStartSec) {
          // 継続時間を最大20サイクル先まで探索
  ...
  // 2. 前方探索: minRepopSec 以降で次の条件区間を探す
    ...
    if (mob.weatherSeedRange || mob.weatherSeedRanges) {
      ...
      for (let tSec = cycleStart; tSec < moonEnd; tSec += WEATHER_CYCLE_SEC) {
        ...
        for (let etSec = etStart; etSec < intersectEnd; etSec += ET_HOUR_SEC) {
          ...
          let windowStart = etSec;
          let windowEnd = Math.min(getEtWindowEnd(mob, etSec), intersectEnd);

          // 継続探索（最大20サイクル）
          let extendCursor = windowEnd;
          let extendCount = 0;

          while (extendCount < 20 && extendCursor < moonEnd) {
            const extendCycleStart = alignToWeatherCycle(extendCursor);
            const extendSeed = getEorzeaWeatherSeed(new Date(extendCycleStart * 1000));
            if (!checkWeatherInRange(mob, extendSeed)) break;
            if (!checkEtCondition(mob, extendCursor)) break;

            const nextEtEnd = Math.min(getEtWindowEnd(mob, extendCursor), moonEnd);
            windowEnd = nextEtEnd;
            extendCursor = nextEtEnd;
            extendCount++;
          }
