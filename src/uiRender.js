...
function createMobCard(mob) {
  ...
  if (mob.Map && mob.spawn_points) {
    ...
    if (remainingCount === 1) {
      ...
      isLastOne = false;
      displayCountText = ` <span class="text-xs text-gray-400 relative -top-0.5">@</span>&nbsp; <span class="text-sm">${remainingCount}<</span>span class="text-xs relative top-[0.04rem]">個</span>`;
    }
