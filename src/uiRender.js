...
            // メンテナンスステータスの更新のために、カードを再作成せずにクラスを更新できるように変更
            const repopInfo = calculateRepop(mob, state.maintenance);
            const isStopped = repopInfo.isMaintenanceStop;
            const stoppedClass = "opacity-50 grayscale pointer-events-none";

            if (isStopped) {
                card.classList.add(...stoppedClass.split(' '));
            } else {
                card.classList.remove(...stoppedClass.split(' '));
            }
