...
      // 連続成立: windowStart は consecutiveStart
...
      // 天候ウィンドウと月齢区間の交差
      // weatherResult.popTime は最短REPOP制約を満たしている
...
          // popTimeは intersectStart をそのまま採用
          return {
            windowStart: intersectStart,
            windowEnd: intersectEnd,
...
        // windowStart が pointSec 以降の場合 (未来の成立開始点を返す)
        if (windowStart >= pointSec) {
          // Case 2: 未来のウィンドウ
          return {
            windowStart,
            windowEnd,
            popTime: windowStart, // ★ ET境界のまま維持
            remainingSec: 0
          };
        }
       }
...
    } else if (status === "Unknown") { // 条件ウィンドウが見つからなかった、または未来ウィンドウだった場合
      status = "PopWindow";
